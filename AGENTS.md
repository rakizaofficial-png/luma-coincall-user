...
- Native projects are not committed — they're generated by `expo prebuild -p android` (config plugins live in `app.json`). `expo-app/android/` and `expo-app/ios/` are gitignored on purpose.
- Two build paths:
  - **EAS (cloud):** `eas build -p android --profile preview` (APK). Requires an Expo account / `EXPO_TOKEN` (not set in this VM).
  - **Local gradle (no KVM needed — building an APK is separate from running an emulator):** install JDK 17 + Android SDK (`cmdline-tools`, `platforms;android-35`, `build-tools;35.0.0`), then `expo prebuild -p android` and `cd android && ./gradlew assembleRelease`. Output: `expo-app/android/app/build/outputs/apk/release/app-release.apk`. The Expo release template debug-signs it, so it's standalone (JS bundled) and installable directly.
- The APK is a WebView shell of `EXPO_PUBLIC_LUMA_WEB_URL` (defaults to the deployed prod site). To ship new web features in the APK, deploy the web app (or point the shell at a reachable build URL) — rebuilding the APK alone won't include unbuilt/undeployed web changes.
- Native networking is already made emulator-safe in code: `expo-app/App.tsx` rewrites `localhost`/`127.0.0.1` → `10.0.2.2` on Android, and `src/config/apiConfig.ts` rewrites a `localhost` API/WS host to the host that actually served the page (so the in-WebView socket/API is reachable from the emulator or a LAN IP). Android cleartext HTTP is enabled via the `expo-build-properties` plugin in `app.json` (`android.usesCleartextTraffic`). The realtime socket logs its lifecycle with `[realtime] …` console lines — the first thing to check when calls work on web but not mobile.
